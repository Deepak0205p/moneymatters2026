import { ModuleSection, TopicSection, SwipeCard } from './types';
...
export function getModuleById(id: number): ModuleSection | undefined {
  return modules.find(m => m.id === id);
}

export function getAllCardsForModule(moduleId: number): SwipeCard[] {
  const mod = getModuleById(moduleId);
  if (!mod) return [];
  return mod.topics.flatMap(topic => topic.cards);
}

export function getTopicById(moduleId: number, topicId: string): TopicSection | undefined {
  const mod = getModuleById(moduleId);
