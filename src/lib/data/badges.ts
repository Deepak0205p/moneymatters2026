...
export type BadgeCategory = 'learning' | 'streak' | 'strategy' | 'special';
...
export interface BadgeData {
  ...
  requirement: string;
  rewardCoins: number;
  /** Estimated % of users who earn this — used to flag "rarest" badges */
  rarity?: number;
}

// Tier color map (for ring + glow effects)
export const TIER_COLORS: Record<BadgeTier, { ring: string; glow: string; label: string }> = {
  bronze: { ring: '#CD7F32', glow: 'rgba(205, 127, 50, 0.45)', label: 'Bronze' },
  silver: { ring: '#C0C0C0', glow: 'rgba(192, 192, 192, 0.45)', label: 'Silver' },
  gold:   { ring: '#F59E0B', glow: 'rgba(245, 158, 11, 0.55)', label: 'Gold' },
  diamond:{ ring: '#A78BFA', glow: 'rgba(167, 139, 250, 0.65)', label: 'Diamond' },
};

export const CATEGORY_META: Record<BadgeCategory, { label: string; emoji: string; accent: string }> = {
...
// ────────────────────────────────────────────────────────────────────────
const LEARNING_BADGES: BadgeData[] = [
  {
    id: 'm1-pehli-seedi',
    name: 'Pehli Seedi',
...
// ────────────────────────────────────────────────────────────────────────
const STRATEGY_BADGES: BadgeData[] = [
  {
...
// ────────────────────────────────────────────────────────────────────────
const SPECIAL_BADGES: BadgeData[] = [
  {
    id: 'first-blood',
    name: 'First Blood',
...
export function getBadgesByCategory(category: BadgeCategory | 'all'): BadgeData[] {
  if (category === 'all') return BADGES;
  return BADGES.filter((b) => b.category === category);
}

// Rarest badges (rarity ≤ 12 — earned by <12% users)
export function getRarestBadges(): BadgeData[] {
...
export interface LevelInfo {
  ...
  emoji: string;
  minXp: number;
  nextLevelXp: number; // XP needed to reach next level (Infinity for max)
}
...
export const LEVEL_NAMES: { name: string; emoji: string }[] = [
...
export function getLevelInfo(xp: number): LevelInfo {
  ...
  const nextLevelXp = level >= MAX_LEVEL ? Infinity : (LEVEL_THRESHOLDS[level] ?? Infinity);
...
export function getLevelProgress(xp: number): {
  ...
  const percent = bandTotal > 0 ? Math.min(100, Math.round((bandCurrent / bandTotal) * 100)) : 100;
  return {
    current: bandCurrent,
    needed: bandTotal,
    percent,
...
// ────────────────────────────────────────────────────────────────────────
export const PROFILE_AVATARS: { id: string; emoji: string; label: string }[] = [
...
export interface ActivityEntry {
  ...
  description: string;
  timestamp: number;
  coins: number; // +ve earned, -ve spent
}
