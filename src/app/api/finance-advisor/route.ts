...
## Context Awareness:
- You may receive context about the user's progress in the RUPAIYA 101 app (coins earned, modules completed, streak days)
- Use this to personalize your advice and encourage them
- Celebrate their progress and suggest next modules/topics to explore`;
...
function getFallbackResponse(message: string): string {
  ...
  if (lower.includes('credit card') || lower.includes('credit card') || lower.includes('emi')) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const recentHistory = (context?.recentMessages || []).slice(-6);
      const historyMessages = recentHistory.map((msg: { role: string; content: string }) => ({
      ...
      console.error('LLM advisor failed:', llmError);
    }
