...
interface Card3DProps {
  children: ReactNode;
  className?: string;
  /** Optional hex color for the glow that follows the cursor (e.g. "#10B981"). */
  glowColor?: string;
  /** Maximum tilt in degrees. Defaults to 8. */
  intensity?: number;
  /** Disables the tilt interaction (still renders children). Defaults to false. */
  disabled?: boolean;
}
...
export function Card3D({
  ...
  const glareBackground = useTransform(
    [glareX, glareY],
    ([gx, gy]: string[]) =>
      `radial-gradient(circle at ${gx} ${gy}, ${glowColor ?? '#10B981'}26, transparent 55%)`,
  );
  ...
  return (
    ...
    >
      ...
      {/* Cursor-tracking glare / glow overlay */}
      {!disabled && glowColor && (
        <motion.div
          aria-hidden
